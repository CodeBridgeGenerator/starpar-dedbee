...
const EmployeeKRAScoreLayout = (props) => {
...
const mapState = (state) => {
    ...
    const { selectedEmployeeKRAScore } = state.employeeKRAScore;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedEmployeeKRAScore, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneEmployeeKRAScore: (id) => dispatch.employeeKRAScore.getOneEmployeeKRAScore(id),
  });

export default connect(mapState, mapDispatch)(EmployeeKRAScoreLayout);
