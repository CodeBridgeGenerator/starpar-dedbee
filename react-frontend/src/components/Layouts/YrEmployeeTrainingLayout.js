...
const YrEmployeeTrainingLayout = (props) => {
...
const mapState = (state) => {
    ...
    const { selectedYrEmployeeTraining } = state.yrEmployeeTraining;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedYrEmployeeTraining, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneYrEmployeeTraining: (id) => dispatch.yrEmployeeTraining.getOneYrEmployeeTraining(id),
  });

export default connect(mapState, mapDispatch)(YrEmployeeTrainingLayout);
