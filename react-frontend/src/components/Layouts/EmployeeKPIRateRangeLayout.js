...
const EmployeeKPIRateRangeLayout = (props) => {
    ...
    const menu = [
        {
            label: 'EmployeeKPIRateRange Mgmt',
            items: [
...
const mapState = (state) => {
    ...
    const { selectedEmployeeKPIRateRange } = state.employeeKPIRateRange;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedEmployeeKPIRateRange, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneEmployeeKPIRateRange: (id) => dispatch.employeeKPIRateRange.getOneEmployeeKPIRateRange(id),
  });

export default connect(mapState, mapDispatch)(EmployeeKPIRateRangeLayout);
