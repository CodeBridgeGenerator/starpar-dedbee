...
const YrEmployeeKRAScoreLayout = (props) => {
...
const mapState = (state) => {
    ...
    const { selectedYrEmployeeKRAScore } = state.yrEmployeeKRAScore;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedYrEmployeeKRAScore, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneYrEmployeeKRAScore: (id) => dispatch.yrEmployeeKRAScore.getOneYrEmployeeKRAScore(id),
  });

export default connect(mapState, mapDispatch)(YrEmployeeKRAScoreLayout);
