...
const EmployeeTrainingLayout = (props) => {
...
const mapState = (state) => {
    ...
    const { selectedEmployeeTraining } = state.employeeTraining;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedEmployeeTraining, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneEmployeeTraining: (id) => dispatch.employeeTraining.getOneEmployeeTraining(id),
  });

export default connect(mapState, mapDispatch)(EmployeeTrainingLayout);
