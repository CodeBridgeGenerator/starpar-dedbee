...
const YrEmployeeAppraisalLayout = (props) => {
    ...
    const menu = [
        {
            label: 'YrEmployeeAppraisal Mgmt',
            items: [
...
const mapState = (state) => {
    ...
    const { selectedYrEmployeeAppraisal } = state.yrEmployeeAppraisal;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedYrEmployeeAppraisal, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneYrEmployeeAppraisal: (id) => dispatch.yrEmployeeAppraisal.getOneYrEmployeeAppraisal(id),
  });

export default connect(mapState, mapDispatch)(YrEmployeeAppraisalLayout);
