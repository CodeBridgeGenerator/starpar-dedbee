...
const EmployeeAppraisalLayout = (props) => {
...
const mapState = (state) => {
    ...
    const { selectedEmployeeAppraisal } = state.employeeAppraisal;
    const { menuOpen } = state.layout;
    return { user, isLoggedIn, selectedEmployeeAppraisal, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneEmployeeAppraisal: (id) => dispatch.employeeAppraisal.getOneEmployeeAppraisal(id),
  });

export default connect(mapState, mapDispatch)(EmployeeAppraisalLayout);
