...
import YrEmployeeCompetenciesscoreDatatable from "./YrEmployeeCompetenciesscoreDataTable";
import YrEmployeeCompetenciesscoreEditDialogComponent from "./YrEmployeeCompetenciesscoreEditDialogComponent";
import YrEmployeeCompetenciesscoreCreateDialogComponent from "./YrEmployeeCompetenciesscoreCreateDialogComponent";
import YrEmployeeCompetenciesscoreFakerDialogComponent from "./YrEmployeeCompetenciesscoreFakerDialogComponent";
import YrEmployeeCompetenciesscoreSeederDialogComponent from "./YrEmployeeCompetenciesscoreSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeCompetenciesscorePage = (props) => {
    ...
    const filename = "yrEmployeeCompetenciesscore";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeCompetenciesscore");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee Competenciesscore", type: "error", message: error.message || "Failed get Yr Employee Competenciesscore" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeCompetenciesscore").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee Competenciesscore", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeCompetenciesscore").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee Competenciesscore",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeCompetenciesscore(data._id);
        navigate(`/yrEmployeeCompetenciesscore/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee Competenciesscore </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeCompetenciesscore-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeCompetenciesscore-datatable">
                    <YrEmployeeCompetenciesscoreDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeCompetenciesscoreEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeCompetenciesscoreCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeCompetenciesscoreFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeCompetenciesscoreSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeCompetenciesscore: (id) => dispatch.yrEmployeeCompetenciesscore.setOneYrEmployeeCompetenciesscore(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeCompetenciesscorePage);
