...
import yrEmployeeCompetenciesscoreFakerFactory from "./yrEmployeeCompetenciesscoreFakerFactory";
...
const YrEmployeeCompetenciesscoreFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeCompetenciesscoreFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeCompetenciesscore").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeCompetenciesscoreFakerDialogComponent);
