...
import employeeTrainingFakerFactory from "./employeeTrainingFakerFactory";
...
const EmployeeTrainingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeTrainingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeTraining").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeTrainingFakerDialogComponent);
