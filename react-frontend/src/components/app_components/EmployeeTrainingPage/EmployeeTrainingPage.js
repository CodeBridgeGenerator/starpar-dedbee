...
import EmployeeTrainingDatatable from "./EmployeeTrainingDataTable";
import EmployeeTrainingEditDialogComponent from "./EmployeeTrainingEditDialogComponent";
import EmployeeTrainingCreateDialogComponent from "./EmployeeTrainingCreateDialogComponent";
import EmployeeTrainingFakerDialogComponent from "./EmployeeTrainingFakerDialogComponent";
import EmployeeTrainingSeederDialogComponent from "./EmployeeTrainingSeederDialogComponent";
...
const EmployeeTrainingPage = (props) => {
    ...
    const filename = "employeeTraining";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("employeeTraining");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Employee Training", type: "error", message: error.message || "Failed get Employee Training" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("employeeTraining").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Employee Training", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("employeeTraining").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        props.setOneEmployeeTraining(data._id);
        navigate(`/employeeTraining/${data._id}`)
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="employeeTraining-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="employeeTraining-datatable">
                    <EmployeeTrainingDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EmployeeTrainingEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EmployeeTrainingCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EmployeeTrainingFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EmployeeTrainingSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneEmployeeTraining: (id) => dispatch.employeeTraining.setOneEmployeeTraining(id)
});

export default connect(mapState, mapDispatch)(EmployeeTrainingPage);
