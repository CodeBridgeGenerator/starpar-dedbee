...
import EmployeeCompetenciesscoreDatatable from "./EmployeeCompetenciesscoreDataTable";
import EmployeeCompetenciesscoreEditDialogComponent from "./EmployeeCompetenciesscoreEditDialogComponent";
import EmployeeCompetenciesscoreCreateDialogComponent from "./EmployeeCompetenciesscoreCreateDialogComponent";
import EmployeeCompetenciesscoreFakerDialogComponent from "./EmployeeCompetenciesscoreFakerDialogComponent";
import EmployeeCompetenciesscoreSeederDialogComponent from "./EmployeeCompetenciesscoreSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const EmployeeCompetenciesscorePage = (props) => {
    ...
    const filename = "employeeCompetenciesscore";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("employeeCompetenciesscore");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Employee Competencies Score", type: "error", message: error.message || "Failed get Employee Competencies Score" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("employeeCompetenciesscore").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Employee Competencies Score", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("employeeCompetenciesscore").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Employee Competencies Score",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneEmployeeCompetenciesscore(data._id);
        navigate(`/employeeCompetenciesscore/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Employee Competencies Score </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="employeeCompetenciesscore-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="employeeCompetenciesscore-datatable">
                    <EmployeeCompetenciesscoreDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EmployeeCompetenciesscoreEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EmployeeCompetenciesscoreCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EmployeeCompetenciesscoreFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EmployeeCompetenciesscoreSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneEmployeeCompetenciesscore: (id) => dispatch.employeeCompetenciesscore.setOneEmployeeCompetenciesscore(id)
});

export default connect(mapState, mapDispatch)(EmployeeCompetenciesscorePage);
