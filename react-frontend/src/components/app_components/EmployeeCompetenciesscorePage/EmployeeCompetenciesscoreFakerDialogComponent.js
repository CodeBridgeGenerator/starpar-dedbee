...
import employeeCompetenciesscoreFakerFactory from "./employeeCompetenciesscoreFakerFactory";
...
const EmployeeCompetenciesscoreFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeCompetenciesscoreFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeCompetenciesscore").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeCompetenciesscoreFakerDialogComponent);
