...
import employeeAppraisalReviewFakerFactory from "./employeeAppraisalReviewFakerFactory";
...
const EmployeeAppraisalReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeAppraisalReviewFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeAppraisalReview").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeAppraisalReviewFakerDialogComponent);
