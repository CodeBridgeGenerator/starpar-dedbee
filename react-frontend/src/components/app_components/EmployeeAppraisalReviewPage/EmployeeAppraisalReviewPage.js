...
import EmployeeAppraisalReviewDatatable from "./EmployeeAppraisalReviewDataTable";
...
import EmployeeAppraisalReviewFakerDialogComponent from "./EmployeeAppraisalReviewFakerDialogComponent";
import EmployeeAppraisalReviewSeederDialogComponent from "./EmployeeAppraisalReviewSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const EmployeeAppraisalReviewPage = (props) => {
    ...
    const filename = "employeeAppraisalReview";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("employeeAppraisalReview");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Employee Appraisal Review", type: "error", message: error.message || "Failed get Employee Appraisal Review" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("employeeAppraisalReview").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Employee Appraisal Review", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("employeeAppraisalReview").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Employee Appraisal Review",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneEmployeeAppraisalReview(data._id);
        navigate(`/employeeAppraisalReview/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Employee Appraisal Review </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="employeeAppraisalReview-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="employeeAppraisalReview-datatable">
                    <EmployeeAppraisalReviewDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EmployeeAppraisalReviewEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EmployeeAppraisalReviewCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EmployeeAppraisalReviewFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EmployeeAppraisalReviewSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneEmployeeAppraisalReview: (id) => dispatch.employeeAppraisalReview.setOneEmployeeAppraisalReview(id)
});

export default connect(mapState, mapDispatch)(EmployeeAppraisalReviewPage);
