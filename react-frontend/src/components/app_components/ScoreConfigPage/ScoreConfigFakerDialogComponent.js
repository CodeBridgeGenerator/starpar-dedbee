...
import scoreConfigFakerFactory from "./scoreConfigFakerFactory";
...
const ScoreConfigFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = scoreConfigFakerFactory(props.user,fakerCount,parSessionIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("scoreConfig").create(elem));
...
export default connect(mapState, mapDispatch)(ScoreConfigFakerDialogComponent);
