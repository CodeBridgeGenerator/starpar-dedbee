...
import ParSessionAppraisersDatatable from "./ParSessionAppraisersDataTable";
import ParSessionAppraisersEditDialogComponent from "./ParSessionAppraisersEditDialogComponent";
import ParSessionAppraisersCreateDialogComponent from "./ParSessionAppraisersCreateDialogComponent";
import ParSessionAppraisersFakerDialogComponent from "./ParSessionAppraisersFakerDialogComponent";
import ParSessionAppraisersSeederDialogComponent from "./ParSessionAppraisersSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ParSessionAppraisersPage = (props) => {
    ...
    const filename = "parSessionAppraisers";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("parSessionAppraisers");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , parSessionId : urlParams.singleParSessionId ,appraiseeEmpNo : urlParams.singleEmployeesId ,appraiserEmpNo : urlParams.singleEmployeesId ,department : urlParams.singleDepartmentsId ,section : urlParams.singleSectionsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Par Session Appraisers", type: "error", message: error.message || "Failed get Par Session Appraisers" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("parSessionAppraisers").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Par Session Appraisers", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("parSessionAppraisers").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Par Session Appraisers",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/parSessionAppraisers/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Par Session Appraisers </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="parSessionAppraisers-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="parSessionAppraisers-datatable">
                    <ParSessionAppraisersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ParSessionAppraisersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ParSessionAppraisersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ParSessionAppraisersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ParSessionAppraisersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ParSessionAppraisersPage);
