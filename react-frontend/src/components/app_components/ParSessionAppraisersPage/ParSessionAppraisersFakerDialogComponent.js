...
import parSessionAppraisersFakerFactory from "./parSessionAppraisersFakerFactory";
...
const ParSessionAppraisersFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [parSessionIdIds, setparSessionIdIds] = useState();
const [appraiseeEmpNoIds, setappraiseeEmpNoIds] = useState();
const [appraiserEmpNoIds, setappraiserEmpNoIds] = useState();
...
const [sectionIds, setsectionIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("parSession")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setparSessionIdIds(data.data);
                        });
client.service("employees")
                        .find({ query: { $select: ["_id","empNo"] } })
                        .then((data) => {
                        setappraiseeEmpNoIds(data.data);
                        });
client.service("employees")
                        .find({ query: { $select: ["_id","empNo"] } })
                        .then((data) => {
                        setappraiserEmpNoIds(data.data);
                        });
...
client.service("sections")
    ...
    const onRun = async () => {
        let fakeData = parSessionAppraisersFakerFactory(props.user,fakerCount,parSessionIdIds,appraiseeEmpNoIds,appraiserEmpNoIds,departmentIds,sectionIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("parSessionAppraisers").create(elem));
...
export default connect(mapState, mapDispatch)(ParSessionAppraisersFakerDialogComponent);
