...
import employeeAppraisalRemarksFakerFactory from "./employeeAppraisalRemarksFakerFactory";
...
const EmployeeAppraisalRemarksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeAppraisalRemarksFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeAppraisalRemarks").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeAppraisalRemarksFakerDialogComponent);
