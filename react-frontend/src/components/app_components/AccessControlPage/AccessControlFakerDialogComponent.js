...
import accessControlFakerFactory from "./accessControlFakerFactory";
...
const AccessControlFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = accessControlFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("accessControl").create(elem));
...
export default connect(mapState, mapDispatch)(AccessControlFakerDialogComponent);
