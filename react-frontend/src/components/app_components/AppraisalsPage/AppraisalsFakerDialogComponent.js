...
import appraisalsFakerFactory from "./appraisalsFakerFactory";
...
const AppraisalsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [employeeNoIds, setemployeeNoIds] = useState();
const [managerEmpNoIds, setmanagerEmpNoIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("employees")
                        ...
                        .then((data) => {
                        setemployeeNoIds(data.data);
                        });
client.service("employees")
                        .find({ query: { $select: ["_id","empNo"] } })
                        .then((data) => {
                        setmanagerEmpNoIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = appraisalsFakerFactory(props.user,fakerCount,employeeNoIds,managerEmpNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("appraisals").create(elem));
...
export default connect(mapState, mapDispatch)(AppraisalsFakerDialogComponent);
