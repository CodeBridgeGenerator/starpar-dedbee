...
import AppraisalsDatatable from "./AppraisalsDataTable";
import AppraisalsEditDialogComponent from "./AppraisalsEditDialogComponent";
import AppraisalsCreateDialogComponent from "./AppraisalsCreateDialogComponent";
import AppraisalsFakerDialogComponent from "./AppraisalsFakerDialogComponent";
import AppraisalsSeederDialogComponent from "./AppraisalsSeederDialogComponent";
...
const AppraisalsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("appraisals");
    ...
    useEffect(() => {
        ...
        client
            .service("appraisals")
            .find({ query: { $limit: 10000 , employeeNo : urlParams.singleEmployeesId ,managerEmpNo : urlParams.singleEmployeesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("appraisals").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Appraisals", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("appraisals").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="appraisals-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="appraisals-datatable">
                    <AppraisalsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AppraisalsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AppraisalsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AppraisalsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AppraisalsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AppraisalsPage);
