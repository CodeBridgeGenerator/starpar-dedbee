...
import employeeKPIRateRangeFakerFactory from "./employeeKPIRateRangeFakerFactory";
...
const EmployeeKPIRateRangeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeKPIRateRangeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeKPIRateRange").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeKPIRateRangeFakerDialogComponent);
