...
import EmployeeKPIRateRangeDatatable from "./EmployeeKPIRateRangeDataTable";
...
import EmployeeKPIRateRangeFakerDialogComponent from "./EmployeeKPIRateRangeFakerDialogComponent";
import EmployeeKPIRateRangeSeederDialogComponent from "./EmployeeKPIRateRangeSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const EmployeeKPIRateRangePage = (props) => {
    ...
    const filename = "employeeKPIRateRange";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("employeeKPIRateRange");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Employee KPI Rate Range", type: "error", message: error.message || "Failed get Employee KPI Rate Range" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("employeeKPIRateRange").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Employee KPI Rate Range", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("employeeKPIRateRange").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Employee KPI Rate Range",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneEmployeeKPIRateRange(data._id);
        navigate(`/employeeKPIRateRange/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Employee KPI Rate Range </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="employeeKPIRateRange-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="employeeKPIRateRange-datatable">
                    <EmployeeKPIRateRangeDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EmployeeKPIRateRangeEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EmployeeKPIRateRangeCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EmployeeKPIRateRangeFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EmployeeKPIRateRangeSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneEmployeeKPIRateRange: (id) => dispatch.employeeKPIRateRange.setOneEmployeeKPIRateRange(id)
});

export default connect(mapState, mapDispatch)(EmployeeKPIRateRangePage);
