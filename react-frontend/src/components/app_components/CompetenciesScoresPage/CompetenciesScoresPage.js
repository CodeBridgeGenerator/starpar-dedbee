...
import CompetenciesScoresDatatable from "./CompetenciesScoresDataTable";
import CompetenciesScoresEditDialogComponent from "./CompetenciesScoresEditDialogComponent";
import CompetenciesScoresCreateDialogComponent from "./CompetenciesScoresCreateDialogComponent";
import CompetenciesScoresFakerDialogComponent from "./CompetenciesScoresFakerDialogComponent";
import CompetenciesScoresSeederDialogComponent from "./CompetenciesScoresSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const CompetenciesScoresPage = (props) => {
    ...
    const filename = "competenciesScores";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("competenciesScores");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , parAppraisersId : urlParams.singleParSessionAppraisersId ,competency : urlParams.singleMasterCompetenciesId ,appraisalId : urlParams.singleAppraisalsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        service : "par_session_appraisers",
                        select:["appraiserEmpNo"]
                    },{
                        path : "competency",
                        service : "master_competencies",
                        select:["name"]
                    },{
                        path : "appraisalId",
                        service : "appraisals",
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "CompetenciesScores", type: "error", message: error.message || "Failed get CompetenciesScores" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("competenciesScores").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "CompetenciesScores", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("competenciesScores").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "CompetenciesScores",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/competenciesScores/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>CompetenciesScores </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="competenciesScores-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="competenciesScores-datatable">
                    <CompetenciesScoresDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CompetenciesScoresEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CompetenciesScoresCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CompetenciesScoresFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CompetenciesScoresSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CompetenciesScoresPage);
