...
const SingleCompetenciesScoresPage = (props) => {
...
const [appraisalId, setAppraisalId] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleCompetenciesScoresId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
                ...
                service: "users",
                select: ["name"],
              },"parAppraisersId","competency","appraisalId"] }})
            .then((res) => {
                set_entity(res || {});
...
const competency = Array.isArray(res.competency)
            ? res.competency.map((elem) => ({ _id: elem._id, name: elem.name }))
            : res.competency
                ? [{ _id: res.competency._id, name: res.competency.name }]
                : [];
...
const appraisalId = Array.isArray(res.appraisalId)
    ...
    }, [props,urlParams.singleCompetenciesScoresId]);


    const goBack = () => {
        navigate("/competenciesScores");
    };
        ...
        <div className="col-12 flex flex-column align-items-center">
            <div className="col-12">
                ...
                <div className="flex align-items-center">
                    ...
                    <h3 className="m-0">CompetenciesScores</h3>
                    <SplitButton
                ...
                {/* <p>competenciesScores/{urlParams.singleCompetenciesScoresId}</p> */}
            </div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">competency</label>
                    {competency.map((elem) => (
                        <Link key={elem._id} to={`/masterCompetencies/${elem._id}`}>
                        <div>
                  {" "}
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">AppraisalId</label>
      ...
      <CommentsSection
        recordId={urlParams.singleCompetenciesScoresId}
        user={props.user}
        alert={props.alert}
...
export default connect(mapState, mapDispatch)(SingleCompetenciesScoresPage);
