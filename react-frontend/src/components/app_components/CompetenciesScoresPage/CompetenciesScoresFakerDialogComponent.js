...
import competenciesScoresFakerFactory from "./competenciesScoresFakerFactory";
...
const CompetenciesScoresFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [parAppraisersIdIds, setparAppraisersIdIds] = useState();
const [competencyIds, setcompetencyIds] = useState();
const [appraisalIdIds, setappraisalIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("parSessionAppraisers")
                        .find({ query: { $select: ["_id","appraiserEmpNo"] } })
                        .then((data) => {
                        setparAppraisersIdIds(data.data);
                        });
client.service("masterCompetencies")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcompetencyIds(data.data);
                        });
client.service("appraisals")
                        .find({ query: { $select: ["_id","employeeNo"] } })
                        .then((data) => {
                        setappraisalIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = competenciesScoresFakerFactory(props.user,fakerCount,parAppraisersIdIds,competencyIds,appraisalIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("competenciesScores").create(elem));
...
export default connect(mapState, mapDispatch)(CompetenciesScoresFakerDialogComponent);
