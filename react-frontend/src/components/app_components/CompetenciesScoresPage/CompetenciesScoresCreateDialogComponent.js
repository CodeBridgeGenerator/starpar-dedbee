...
const CompetenciesScoresCreateDialogComponent = (props) => {
...
const [appraisalId, setAppraisalId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [parAppraisersId,competency,appraisalId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            parAppraisersId: _entity?.parAppraisersId?._id,competency: _entity?.competency?._id,rate: _entity?.rate,weightedRate: _entity?.weightedRate,remarks: _entity?.remarks,appraisalId: _entity?.appraisalId?._id,
        ...
        const result = await client.service("competenciesScores").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info CompetenciesScores updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in CompetenciesScores" });
        }
        setLoading(false);
    };
...
const appraisalIdOptions = appraisalId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create CompetenciesScores" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="competenciesScores-create-dialog-component">
...
export default connect(mapState, mapDispatch)(CompetenciesScoresCreateDialogComponent);
