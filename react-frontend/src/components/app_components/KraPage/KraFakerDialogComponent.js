...
import kraFakerFactory from "./kraFakerFactory";
...
const KraFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = kraFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("kra").create(elem));
...
export default connect(mapState, mapDispatch)(KraFakerDialogComponent);
