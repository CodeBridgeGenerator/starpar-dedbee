...
import kraScoresFakerFactory from "./kraScoresFakerFactory";
...
const KraScoresFakerDialogComponent = (props) => {
...
client.service("appraisals")
    ...
    const onRun = async () => {
        let fakeData = kraScoresFakerFactory(props.user,fakerCount,parAppraisersIdIds,kpiIds,appraisalIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("kraScores").create(elem));
...
export default connect(mapState, mapDispatch)(KraScoresFakerDialogComponent);
