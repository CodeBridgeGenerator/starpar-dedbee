...
const KraScoresEditDialogComponent = (props) => {
...
appraisalId: _entity?.appraisalId?._id,
        ...
        await client.service("kraScores").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info kraScores updated successfully" });
...
const appraisalIdOptions = appraisalId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit KraScores" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="kraScores-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="score" className="w-full mb-3 p-inputtext-sm" value={_entity?.score} onChange={(e) => setValByKey("score", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(KraScoresEditDialogComponent);
