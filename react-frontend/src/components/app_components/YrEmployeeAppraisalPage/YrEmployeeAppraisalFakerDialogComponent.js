...
import yrEmployeeAppraisalFakerFactory from "./yrEmployeeAppraisalFakerFactory";
...
const YrEmployeeAppraisalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeAppraisalFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeAppraisal").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeAppraisalFakerDialogComponent);
