...
import YrEmployeeAppraisalDatatable from "./YrEmployeeAppraisalDataTable";
import YrEmployeeAppraisalEditDialogComponent from "./YrEmployeeAppraisalEditDialogComponent";
import YrEmployeeAppraisalCreateDialogComponent from "./YrEmployeeAppraisalCreateDialogComponent";
import YrEmployeeAppraisalFakerDialogComponent from "./YrEmployeeAppraisalFakerDialogComponent";
import YrEmployeeAppraisalSeederDialogComponent from "./YrEmployeeAppraisalSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeAppraisalPage = (props) => {
    ...
    const filename = "yrEmployeeAppraisal";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeAppraisal");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee Appraisal", type: "error", message: error.message || "Failed get Yr Employee Appraisal" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeAppraisal").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee Appraisal", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeAppraisal").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee Appraisal",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeAppraisal(data._id);
        navigate(`/yrEmployeeAppraisal/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee Appraisal </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeAppraisal-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeAppraisal-datatable">
                    <YrEmployeeAppraisalDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeAppraisalEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeAppraisalCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeAppraisalFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeAppraisalSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeAppraisal: (id) => dispatch.yrEmployeeAppraisal.setOneYrEmployeeAppraisal(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeAppraisalPage);
