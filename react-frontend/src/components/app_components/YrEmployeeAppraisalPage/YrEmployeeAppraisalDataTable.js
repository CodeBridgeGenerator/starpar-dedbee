...
const YrEmployeeAppraisalDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
<Column field="competencyWeight" header=" Competency Weight" body={p_inputnumberTemplate19} filter={selectedFilterFields.includes("competencyWeight")} hidden={selectedHideFields?.includes("competencyWeight")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload YrEmployeeAppraisal Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search YrEmployeeAppraisal" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default YrEmployeeAppraisalDataTable;