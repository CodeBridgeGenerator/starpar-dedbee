...
import masterCompetenciesFakerFactory from "./masterCompetenciesFakerFactory";
...
const MasterCompetenciesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = masterCompetenciesFakerFactory(props.user,fakerCount,departmentIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("masterCompetencies").create(elem));
...
export default connect(mapState, mapDispatch)(MasterCompetenciesFakerDialogComponent);
