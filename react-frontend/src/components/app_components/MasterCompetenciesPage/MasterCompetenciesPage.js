...
import MasterCompetenciesDatatable from "./MasterCompetenciesDataTable";
import MasterCompetenciesEditDialogComponent from "./MasterCompetenciesEditDialogComponent";
import MasterCompetenciesCreateDialogComponent from "./MasterCompetenciesCreateDialogComponent";
import MasterCompetenciesFakerDialogComponent from "./MasterCompetenciesFakerDialogComponent";
import MasterCompetenciesSeederDialogComponent from "./MasterCompetenciesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MasterCompetenciesPage = (props) => {
    ...
    const filename = "masterCompetencies";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("masterCompetencies");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Master Competencies", type: "error", message: error.message || "Failed get Master Competencies" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("masterCompetencies").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Master Competencies", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("masterCompetencies").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/masterCompetencies/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Master Competencies </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="masterCompetencies-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="masterCompetencies-datatable">
                    <MasterCompetenciesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MasterCompetenciesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MasterCompetenciesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MasterCompetenciesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MasterCompetenciesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MasterCompetenciesPage);
