...
const MasterCompetenciesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            departmentId: _entity?.departmentId?._id,name: _entity?.name,description: _entity?.description,weight: _entity?.weight,rate: _entity?.rate,weightedScore: _entity?.weightedScore,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Master Competencies updated successfully" });
