...
import competenciesFakerFactory from "./competenciesFakerFactory";
...
const CompetenciesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = competenciesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("competencies").create(elem));
...
export default connect(mapState, mapDispatch)(CompetenciesFakerDialogComponent);
