...
const CompetenciesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            parId: _entity?.parId,appraisorId: _entity?.appraisorId,appraiseeId: _entity?.appraiseeId,name: _entity?.name,description: _entity?.description,weight: _entity?.weight,rate: _entity?.rate,weightedScore: _entity?.weightedScore,
        ...
        const result = await client.service("competencies").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Competencies created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Competencies" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Competencies" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="competencies-create-dialog-component">
...
export default connect(mapState, mapDispatch)(CompetenciesCreateDialogComponent);
