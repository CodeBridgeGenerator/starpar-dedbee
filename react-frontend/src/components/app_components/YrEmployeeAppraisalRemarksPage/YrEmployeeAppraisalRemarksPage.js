...
import YrEmployeeAppraisalRemarksDatatable from "./YrEmployeeAppraisalRemarksDataTable";
...
import YrEmployeeAppraisalRemarksFakerDialogComponent from "./YrEmployeeAppraisalRemarksFakerDialogComponent";
import YrEmployeeAppraisalRemarksSeederDialogComponent from "./YrEmployeeAppraisalRemarksSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeAppraisalRemarksPage = (props) => {
    ...
    const filename = "yrEmployeeAppraisalRemarks";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeAppraisalRemarks");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee Appraisal Remarks", type: "error", message: error.message || "Failed get Yr Employee Appraisal Remarks" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeAppraisalRemarks").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee Appraisal Remarks", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeAppraisalRemarks").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee Appraisal Remarks",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeAppraisalRemark(data._id);
        navigate(`/yrEmployeeAppraisalRemarks/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee Appraisal Remarks </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeAppraisalRemarks-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeAppraisalRemarks-datatable">
                    <YrEmployeeAppraisalRemarksDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeAppraisalRemarksEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeAppraisalRemarksCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeAppraisalRemarksFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeAppraisalRemarksSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeAppraisalRemark: (id) => dispatch.yrEmployeeAppraisalRemark.setOneYrEmployeeAppraisalRemark(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeAppraisalRemarksPage);
