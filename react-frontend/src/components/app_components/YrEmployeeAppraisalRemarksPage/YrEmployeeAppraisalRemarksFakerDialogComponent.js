...
import yrEmployeeAppraisalRemarksFakerFactory from "./yrEmployeeAppraisalRemarksFakerFactory";
...
const YrEmployeeAppraisalRemarksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeAppraisalRemarksFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeAppraisalRemarks").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeAppraisalRemarksFakerDialogComponent);
