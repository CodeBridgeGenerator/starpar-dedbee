...
import calibrationFakerFactory from "./calibrationFakerFactory";
...
const CalibrationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = calibrationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("calibration").create(elem));
...
export default connect(mapState, mapDispatch)(CalibrationFakerDialogComponent);
