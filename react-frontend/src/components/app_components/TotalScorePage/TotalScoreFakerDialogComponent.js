...
import totalScoreFakerFactory from "./totalScoreFakerFactory";
...
const TotalScoreFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = totalScoreFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("totalScore").create(elem));
...
export default connect(mapState, mapDispatch)(TotalScoreFakerDialogComponent);
