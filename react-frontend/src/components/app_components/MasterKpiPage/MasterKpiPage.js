...
import MasterKpiDatatable from "./MasterKpiDataTable";
import MasterKpiEditDialogComponent from "./MasterKpiEditDialogComponent";
import MasterKpiCreateDialogComponent from "./MasterKpiCreateDialogComponent";
import MasterKpiFakerDialogComponent from "./MasterKpiFakerDialogComponent";
import MasterKpiSeederDialogComponent from "./MasterKpiSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MasterKpiPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("masterKpi");
    ...
    useEffect(() => {
        ...
        client
            .service("masterKpi")
            .find({ query: { $limit: 10000 , kra : urlParams.singleMasterKraId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Master KPI", type: "error", message: error.message || "Failed get Master KPI" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("masterKpi").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Master KPI", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("masterKpi").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="masterKpi-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="masterKpi-datatable">
                    <MasterKpiDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MasterKpiEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MasterKpiCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MasterKpiFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MasterKpiSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MasterKpiPage);
