...
import masterKpiFakerFactory from "./masterKpiFakerFactory";
...
const MasterKpiFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = masterKpiFakerFactory(props.user,fakerCount,kraIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("masterKpi").create(elem));
...
export default connect(mapState, mapDispatch)(MasterKpiFakerDialogComponent);
