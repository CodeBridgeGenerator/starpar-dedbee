...
import masterKraFakerFactory from "./masterKraFakerFactory";
...
const MasterKraFakerDialogComponent = (props) => {
...
const [balancedScorecardIds, setbalancedScorecardIds] = useState();
...
client.service("masterBsc")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setbalancedScorecardIds(data.data);
                        });
client.service("sections")
    ...
    const onRun = async () => {
        let fakeData = masterKraFakerFactory(props.user,fakerCount,departmentIdIds,parSessionIdIds,balancedScorecardIds,sectionIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("masterKra").create(elem));
...
export default connect(mapState, mapDispatch)(MasterKraFakerDialogComponent);
