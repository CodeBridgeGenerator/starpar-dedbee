...
const MasterKraCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            departmentId: _entity?.departmentId,balancedScorecard: _entity?.balancedScorecard,kpi: _entity?.kpi,weight: _entity?.weight,target: _entity?.target,actualAchievement: _entity?.actualAchievement,rate: _entity?.rate,weightedRate: _entity?.weightedRate,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Master Kra created successfully" });
