...
const MasterKraEditDialogComponent = (props) => {
...
weightedRate: _entity?.weightedRate,
        ...
        const result = await client.service("masterKra").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info masterKra updated successfully" });
    ...
    return (
        <Dialog header="Edit Master Kra" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="masterKra-edit-dialog-component">
                <div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="departmentId">DepartmentId:</label>
                <InputNumber id="departmentId" className="w-full mb-3 p-inputtext-sm" value={_entity?.departmentId} onChange={(e) => setValByKey("departmentId", e.value)}  useGrouping={false}/>
            </span>
...
export default connect(mapState, mapDispatch)(MasterKraEditDialogComponent);
