...
import YrEmployeeKPIRateRangeDatatable from "./YrEmployeeKPIRateRangeDataTable";
...
import YrEmployeeKPIRateRangeFakerDialogComponent from "./YrEmployeeKPIRateRangeFakerDialogComponent";
import YrEmployeeKPIRateRangeSeederDialogComponent from "./YrEmployeeKPIRateRangeSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeKPIRateRangePage = (props) => {
    ...
    const filename = "yrEmployeeKPIRateRange";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeKPIRateRange");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee K P I Rate Range", type: "error", message: error.message || "Failed get Yr Employee K P I Rate Range" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeKPIRateRange").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee K P I Rate Range", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeKPIRateRange").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee K P I Rate Range",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeKPIRateRange(data._id);
        navigate(`/yrEmployeeKPIRateRange/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee K P I Rate Range </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeKPIRateRange-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeKPIRateRange-datatable">
                    <YrEmployeeKPIRateRangeDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeKPIRateRangeEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeKPIRateRangeCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeKPIRateRangeFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeKPIRateRangeSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeKPIRateRange: (id) => dispatch.yrEmployeeKPIRateRange.setOneYrEmployeeKPIRateRange(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeKPIRateRangePage);
