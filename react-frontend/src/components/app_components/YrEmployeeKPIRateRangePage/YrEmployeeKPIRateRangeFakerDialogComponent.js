...
import yrEmployeeKPIRateRangeFakerFactory from "./yrEmployeeKPIRateRangeFakerFactory";
...
const YrEmployeeKPIRateRangeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeKPIRateRangeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeKPIRateRange").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeKPIRateRangeFakerDialogComponent);
