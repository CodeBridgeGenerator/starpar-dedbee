...
import YrEmployeeTrainingDatatable from "./YrEmployeeTrainingDataTable";
import YrEmployeeTrainingEditDialogComponent from "./YrEmployeeTrainingEditDialogComponent";
import YrEmployeeTrainingCreateDialogComponent from "./YrEmployeeTrainingCreateDialogComponent";
import YrEmployeeTrainingFakerDialogComponent from "./YrEmployeeTrainingFakerDialogComponent";
import YrEmployeeTrainingSeederDialogComponent from "./YrEmployeeTrainingSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeTrainingPage = (props) => {
    ...
    const filename = "yrEmployeeTraining";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeTraining");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee Training", type: "error", message: error.message || "Failed get Yr Employee Training" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeTraining").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee Training", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeTraining").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee Training",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeTraining(data._id);
        navigate(`/yrEmployeeTraining/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee Training </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeTraining-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeTraining-datatable">
                    <YrEmployeeTrainingDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeTrainingEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeTrainingCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeTrainingFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeTrainingSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeTraining: (id) => dispatch.yrEmployeeTraining.setOneYrEmployeeTraining(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeTrainingPage);
