...
import yrEmployeeTrainingFakerFactory from "./yrEmployeeTrainingFakerFactory";
...
const YrEmployeeTrainingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeTrainingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeTraining").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeTrainingFakerDialogComponent);
