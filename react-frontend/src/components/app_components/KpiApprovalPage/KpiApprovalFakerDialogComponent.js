...
import kpiApprovalFakerFactory from "./kpiApprovalFakerFactory";
...
const KpiApprovalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = kpiApprovalFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("kpiApproval").create(elem));
...
export default connect(mapState, mapDispatch)(KpiApprovalFakerDialogComponent);
