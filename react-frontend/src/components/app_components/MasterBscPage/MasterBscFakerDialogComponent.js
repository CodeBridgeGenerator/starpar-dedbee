...
import masterBscFakerFactory from "./masterBscFakerFactory";
...
const MasterBscFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = masterBscFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("masterBsc").create(elem));
...
export default connect(mapState, mapDispatch)(MasterBscFakerDialogComponent);
