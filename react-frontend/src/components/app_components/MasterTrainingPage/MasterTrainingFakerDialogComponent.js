...
import masterTrainingFakerFactory from "./masterTrainingFakerFactory";
...
const MasterTrainingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = masterTrainingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("masterTraining").create(elem));
...
export default connect(mapState, mapDispatch)(MasterTrainingFakerDialogComponent);
