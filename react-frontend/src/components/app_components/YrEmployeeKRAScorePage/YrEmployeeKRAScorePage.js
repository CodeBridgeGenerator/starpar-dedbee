...
import YrEmployeeKRAScoreDatatable from "./YrEmployeeKRAScoreDataTable";
import YrEmployeeKRAScoreEditDialogComponent from "./YrEmployeeKRAScoreEditDialogComponent";
import YrEmployeeKRAScoreCreateDialogComponent from "./YrEmployeeKRAScoreCreateDialogComponent";
import YrEmployeeKRAScoreFakerDialogComponent from "./YrEmployeeKRAScoreFakerDialogComponent";
import YrEmployeeKRAScoreSeederDialogComponent from "./YrEmployeeKRAScoreSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeKRAScorePage = (props) => {
    ...
    const filename = "yrEmployeeKRAScore";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeKRAScore");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee K R A Score", type: "error", message: error.message || "Failed get Yr Employee K R A Score" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeKRAScore").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee K R A Score", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeKRAScore").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee K R A Score",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeKRAScore(data._id);
        navigate(`/yrEmployeeKRAScore/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee K R A Score </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeKRAScore-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeKRAScore-datatable">
                    <YrEmployeeKRAScoreDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeKRAScoreEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeKRAScoreCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeKRAScoreFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeKRAScoreSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeKRAScore: (id) => dispatch.yrEmployeeKRAScore.setOneYrEmployeeKRAScore(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeKRAScorePage);
