...
import yrEmployeeKRAScoreFakerFactory from "./yrEmployeeKRAScoreFakerFactory";
...
const YrEmployeeKRAScoreFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeKRAScoreFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeKRAScore").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeKRAScoreFakerDialogComponent);
