...
const ParSessionDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_dateTemplate2 = (rowData, { rowIndex }) => <p >{moment(rowData.startDate).fromNow()}</p>
const p_dateTemplate3 = (rowData, { rowIndex }) => <p >{moment(rowData.endDate).fromNow()}</p>
...
<Column field="startDate" header="Start Date" body={p_dateTemplate2} filter={selectedFilterFields.includes("startDate")} hidden={selectedHideFields?.includes("startDate")}  sortable style={{ minWidth: "8rem" }} />
<Column field="endDate" header="End Date" body={p_dateTemplate3} filter={selectedFilterFields.includes("endDate")} hidden={selectedHideFields?.includes("endDate")}  sortable style={{ minWidth: "8rem" }} />
<Column field="status" header="Status" body={pTemplate4} filter={selectedFilterFields.includes("status")} hidden={selectedHideFields?.includes("status")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload ParSession Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="parSession"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search ParSession" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ParSessionDataTable;