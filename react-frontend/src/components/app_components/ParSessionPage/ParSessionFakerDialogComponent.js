...
import parSessionFakerFactory from "./parSessionFakerFactory";
...
const ParSessionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = parSessionFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("parSession").create(elem));
...
export default connect(mapState, mapDispatch)(ParSessionFakerDialogComponent);
