...
import YrEmployeeAppraisalReviewDatatable from "./YrEmployeeAppraisalReviewDataTable";
...
import YrEmployeeAppraisalReviewFakerDialogComponent from "./YrEmployeeAppraisalReviewFakerDialogComponent";
import YrEmployeeAppraisalReviewSeederDialogComponent from "./YrEmployeeAppraisalReviewSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const YrEmployeeAppraisalReviewPage = (props) => {
    ...
    const filename = "yrEmployeeAppraisalReview";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("yrEmployeeAppraisalReview");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Yr Employee Appraisal Review", type: "error", message: error.message || "Failed get Yr Employee Appraisal Review" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("yrEmployeeAppraisalReview").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Yr Employee Appraisal Review", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("yrEmployeeAppraisalReview").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Yr Employee Appraisal Review",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneYrEmployeeAppraisalReview(data._id);
        navigate(`/yrEmployeeAppraisalReview/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Yr Employee Appraisal Review </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="yrEmployeeAppraisalReview-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="yrEmployeeAppraisalReview-datatable">
                    <YrEmployeeAppraisalReviewDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <YrEmployeeAppraisalReviewEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <YrEmployeeAppraisalReviewCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <YrEmployeeAppraisalReviewFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <YrEmployeeAppraisalReviewSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneYrEmployeeAppraisalReview: (id) => dispatch.yrEmployeeAppraisalReview.setOneYrEmployeeAppraisalReview(id)
});

export default connect(mapState, mapDispatch)(YrEmployeeAppraisalReviewPage);
