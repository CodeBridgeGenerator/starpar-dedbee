...
import yrEmployeeAppraisalReviewFakerFactory from "./yrEmployeeAppraisalReviewFakerFactory";
...
const YrEmployeeAppraisalReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = yrEmployeeAppraisalReviewFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("yrEmployeeAppraisalReview").create(elem));
...
export default connect(mapState, mapDispatch)(YrEmployeeAppraisalReviewFakerDialogComponent);
