...
import personalDevelopmentFakerFactory from "./personalDevelopmentFakerFactory";
...
const PersonalDevelopmentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = personalDevelopmentFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("personalDevelopment").create(elem));
...
export default connect(mapState, mapDispatch)(PersonalDevelopmentFakerDialogComponent);
