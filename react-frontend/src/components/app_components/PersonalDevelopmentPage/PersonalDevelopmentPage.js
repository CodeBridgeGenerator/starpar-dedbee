...
import PersonalDevelopmentDatatable from "./PersonalDevelopmentDataTable";
...
import PersonalDevelopmentFakerDialogComponent from "./PersonalDevelopmentFakerDialogComponent";
import PersonalDevelopmentSeederDialogComponent from "./PersonalDevelopmentSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const PersonalDevelopmentPage = (props) => {
    ...
    const filename = "personalDevelopment";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("personalDevelopment");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Personal Development", type: "error", message: error.message || "Failed get Personal Development" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("personalDevelopment").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Personal Development", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("personalDevelopment").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/personalDevelopment/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="personalDevelopment-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="personalDevelopment-datatable">
                    <PersonalDevelopmentDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <PersonalDevelopmentEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PersonalDevelopmentCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <PersonalDevelopmentFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PersonalDevelopmentSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PersonalDevelopmentPage);
