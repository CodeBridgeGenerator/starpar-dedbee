...
const PersonalDevelopmentCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            appraiseeId: _entity?.appraiseeId,strengthsAndAreasOfConcernIdentified: _entity?.strengthsAndAreasOfConcernIdentified,specialAchievementsForThePeriodUnderReview: _entity?.specialAchievementsForThePeriodUnderReview,
        ...
        const result = await client.service("personalDevelopment").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Personal Development created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Personal Development" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Personal Development" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="personalDevelopment-create-dialog-component">
...
export default connect(mapState, mapDispatch)(PersonalDevelopmentCreateDialogComponent);
