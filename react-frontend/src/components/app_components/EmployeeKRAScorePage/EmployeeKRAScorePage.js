...
import EmployeeKRAScoreDatatable from "./EmployeeKRAScoreDataTable";
import EmployeeKRAScoreEditDialogComponent from "./EmployeeKRAScoreEditDialogComponent";
import EmployeeKRAScoreCreateDialogComponent from "./EmployeeKRAScoreCreateDialogComponent";
import EmployeeKRAScoreFakerDialogComponent from "./EmployeeKRAScoreFakerDialogComponent";
import EmployeeKRAScoreSeederDialogComponent from "./EmployeeKRAScoreSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const EmployeeKRAScorePage = (props) => {
    ...
    const filename = "employeeKRAScore";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("employeeKRAScore");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Employee KRA Score", type: "error", message: error.message || "Failed get Employee KRA Score" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("employeeKRAScore").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Employee KRA Score", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("employeeKRAScore").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Employee KRA Score",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        props.setOneEmployeeKRAScore(data._id);
        navigate(`/employeeKRAScore/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Employee KRA Score </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="employeeKRAScore-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="employeeKRAScore-datatable">
                    <EmployeeKRAScoreDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EmployeeKRAScoreEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EmployeeKRAScoreCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EmployeeKRAScoreFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EmployeeKRAScoreSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneEmployeeKRAScore: (id) => dispatch.employeeKRAScore.setOneEmployeeKRAScore(id)
});

export default connect(mapState, mapDispatch)(EmployeeKRAScorePage);
