...
import employeeKRAScoreFakerFactory from "./employeeKRAScoreFakerFactory";
...
const EmployeeKRAScoreFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeKRAScoreFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeKRAScore").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeKRAScoreFakerDialogComponent);
