...
const RateReviewCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            parId: _entity?.parId,appraisorId: _entity?.appraisorId,appraiseeId: _entity?.appraiseeId,kpiScore: _entity?.kpiScore,competency: _entity?.competency,totalScore: _entity?.totalScore,status: _entity?.status,
        ...
        const result = await client.service("rateReview").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Rate & Review created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Rate & Review" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Rate & Review" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="rateReview-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="kpiScore" className="w-full mb-3 p-inputtext-sm" value={_entity?.kpiScore} onChange={(e) => setValByKey("kpiScore", e.value)}  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(RateReviewCreateDialogComponent);
