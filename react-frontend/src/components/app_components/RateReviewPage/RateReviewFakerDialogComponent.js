...
import rateReviewFakerFactory from "./rateReviewFakerFactory";
...
const RateReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = rateReviewFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("rateReview").create(elem));
...
export default connect(mapState, mapDispatch)(RateReviewFakerDialogComponent);
