...
import employeeAppraisalFakerFactory from "./employeeAppraisalFakerFactory";
...
const EmployeeAppraisalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeAppraisalFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeAppraisal").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeAppraisalFakerDialogComponent);
