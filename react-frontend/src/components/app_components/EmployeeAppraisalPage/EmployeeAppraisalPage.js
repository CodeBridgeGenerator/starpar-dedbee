...
import EmployeeAppraisalDatatable from "./EmployeeAppraisalDataTable";
...
import EmployeeAppraisalFakerDialogComponent from "./EmployeeAppraisalFakerDialogComponent";
import EmployeeAppraisalSeederDialogComponent from "./EmployeeAppraisalSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const EmployeeAppraisalPage = (props) => {
    ...
    const filename = "employeeAppraisal";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("employeeAppraisal");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Employee Appraisal", type: "error", message: error.message || "Failed get Employee Appraisal" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("employeeAppraisal").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Employee Appraisal", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("employeeAppraisal").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        props.setOneEmployeeAppraisal(data._id);
        navigate(`/employeeAppraisal/${data._id}`)
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Employee Appraisal </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="employeeAppraisal-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="employeeAppraisal-datatable">
                    <EmployeeAppraisalDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <EmployeeAppraisalEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <EmployeeAppraisalCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <EmployeeAppraisalFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <EmployeeAppraisalSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
const mapDispatch = (dispatch) => ({
    ...
    setOneEmployeeAppraisal: (id) => dispatch.employeeAppraisal.setOneEmployeeAppraisal(id)
});

export default connect(mapState, mapDispatch)(EmployeeAppraisalPage);
