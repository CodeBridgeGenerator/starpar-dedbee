...
describe("employeeAppraisal service", () => {
  ...
  describe("#create", () => {
    const options = {"appraisalID":23,"employeeNo":"new value","staffName":"new value","fYear":23,"startDate":1769369338757,"endDate":1769369338757,"kRAScore":23,"weightedKRAScore":23,"competencyScore":23,"weightedCompetencyScore":23,"demeritScore":23,"totalScore":23,"finalScore":23,"empComment":"new value","mngComment":"new value","mngSuperiorComment":"new value","managerEmpNO":"new value","managerName":"new value","kRAWeight":23,"competencyWeight":23};

    beforeEach(async () => {
      employeeAppraisalCreated = await thisService.create(options);
    });
...
assert.strictEqual(employeeAppraisalCreated.competencyWeight, options.competencyWeight);
  ...
  describe("#get", () => {
    it("should retrieve a employeeAppraisal by ID", async () => {
      const retrieved = await thisService.get(employeeAppraisalCreated._id);
      assert.strictEqual(retrieved._id, employeeAppraisalCreated._id);
    });
  });

  describe("#update", () => {
    let employeeAppraisalUpdated;
    ...
    beforeEach(async () => {
      employeeAppraisalUpdated = await thisService.update(employeeAppraisalCreated._id, options);
    });
...
assert.strictEqual(employeeAppraisalUpdated.competencyWeight, options.competencyWeight);
  ...
  describe("#delete", () => {
  let employeeAppraisalDeleted;
    beforeEach(async () => {
      employeeAppraisalDeleted = await thisService.remove(employeeAppraisalCreated._id);
    });

    it("should delete a employeeAppraisal", async () => {
      assert.strictEqual(employeeAppraisalDeleted._id, employeeAppraisalCreated._id);
    });
  });
});